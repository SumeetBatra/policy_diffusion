...
class MLP_GHN(nn.Module):
    ...
    def __init__(self,
        ...
        for l in self.model_shape:
            self.model_shape_indicators.append(torch.tensor(l).type(torch.FloatTensor).to(device))
            self.model_shape_indicators.append(torch.tensor(l).type(torch.FloatTensor).to(device))
    ...
    def forward(self, nets_torch, z, y=None):
        r"""
        ...
        for i in range(len(nets_torch)):
            graph_edges = copy.deepcopy(self.default_edges[0:len(
                list((nets_torch if isinstance(nets_torch, list) else [nets_torch])[i].named_parameters()))])
    ...
    def _map_net_params(self, nets_torch, sanity_check=False):
        ...
        for j in range(len(nets_torch)):
            ...
            node_infos.append([[(i + 1, name, 'conv' if 'weight' in name else 'bias',
                                 (3, 16, 1, 1) if 'weight' in name else None,
                                 True if i == (len_node_info - 2) else False,
                                 True if i == (len_node_info - 1) else False) for i, (name, param) in
                                enumerate(nets_torch[j].named_parameters())]])
    ...
    def __init__(self, hid, num_observations, num_classes, max_shape, debug_level=0):
        ...
        for c in range(4, self.ch_steps[0]):
            self.channels_lookup[c] = self.channels_lookup[
                self.ch_steps[0]]  # 4-7 channels will be treated as 8 channels
        ...
        for t in range(self.T):
            for order in traversal_orders:  # forward, backward
                ...
                for node in (all_nodes if order else torch.flipud(all_nodes)):
                    ...
                    m = torch.zeros(B, C, dtype=m.dtype, device=m.device).scatter_add_(0, edge_graph_ind[e_1hop],
                                                                                       m)  # sum the transformed features into a (B,C) tensor
                    if self.ve:
                        e = torch.nonzero(masks_all[order][node, :]).view(-1)  # virtual edges connected to node
                        m_ve = self.mlp_ve(hx[start[e]]) / ve[e].to(
                            m)  # transform node features of all ve-hop neighbors
